          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
            ...
            {lead.url ? (
              <a href={lead.url} target="_blank" rel="noopener noreferrer"
                style={{ fontSize: 11, color: 'var(--ink-3)', textDecoration: 'none', borderBottom: '1px solid rgba(0,0,0,0.15)', paddingBottom: 1 }}>
            ...
            {lead.gold_matches.length > 0 && (
              <div style={{ fontSize: 12, color: 'var(--ink-3)' }}>
                ...
                {lead.gold_matches.map((m, i) => (
                  <span key={i}>
                    <span style={{ color: 'var(--gold)', fontWeight: 500 }}>{m.title} ({m.pct}%)</span>
                    {i < lead.gold_matches.length - 1 ? ' · ' : ''}
                  </span>
                ))}
              </div>
            )}
